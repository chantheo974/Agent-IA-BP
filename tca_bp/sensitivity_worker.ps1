...
function CloseBook {
    ...
    if ($null -ne $script:book) {
        ...
        [void][Runtime.InteropServices.Marshal]::FinalReleaseComObject($script:book); $script:book = $null
    }
}
function OpenBook($path) {
    ...
    $script:sheet = $book.Worksheets.Item('Sensi Analyses')
}
...
function AssertDrivers($expected) {
    foreach ($address in @('C8','C14','C18')) {
        if ($sheet.Range($address).HasFormula -or -not $sheet.Range($address).Locked -or
